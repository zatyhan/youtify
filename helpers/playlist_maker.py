...
class PlaylistMaker():
    def __init__(self, cache_handler):
        ...
        self.__auth_manager__ = SpotifyOAuth(client_id=self.__clientID__, redirect_uri=f"http://127.0.0.1:{os.getenv('PORT')}/callback", scope=self.__scope__, cache_handler=cache_handler, show_dialog=True)
        self.user_id= None
        self.__sp__= None
